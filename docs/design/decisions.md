...
| D-019 | 2026-07-18 | Map data: custom JSON (`data/map/port.json`) authored from the ASCII grid in `docs/design/map-layout.md`; not Tiled | One small hand-authored map; no build chain; avoids Tiled parsing complexity (requirements Appendix A item resolved) | Tiled JSON (tooling overhead unjustified for a single 40×30 map) |
